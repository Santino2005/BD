...
import edu.austral.dissis.chess.engine.Chess.MoveConditions.Conditions.KingOnSquare;
...
public class KingRestriction implements ValidConditions {
    @Override
    public boolean valid(MoveTo moveTo, Board board) {
        int positionRow = moveTo.getTo().row();
        int positionCol = moveTo.getTo().column();
        Map<Position, Piece> simulatedBoard = new HashMap<>(board.getCells());
