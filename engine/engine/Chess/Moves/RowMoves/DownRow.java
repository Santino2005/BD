package edu.austral.dissis.chess.engine.Chess.Moves.RowMoves;
...
public class DownRow implements Valuate {

    private final List<MovesValidator> movesValidator;

    public DownRow(MovesValidator... movesValidator) {
        this.movesValidator = Arrays.asList(movesValidator);
    }

    @Override
    public boolean valuate(MoveTo moveTo, Board board) {
        if (moveTo.getFrom().column() == moveTo.getTo().column()
                && moveTo.getFrom().row() > moveTo.getTo().row()){
    ...
    private List<Position> checkWays(MoveTo initialMove, Board board) {
        List<Position> list = new ArrayList<>();
        int currentRow = initialMove.getFrom().row() - 1;
        int currentCol = initialMove.getFrom().column();
        Position actualPosition = new Position(currentRow, currentCol);

        while (board.validPosition(new MoveTo(initialMove.getFrom(), actualPosition))) {
            MoveTo tempMove = new MoveTo(initialMove.getFrom(), actualPosition);
