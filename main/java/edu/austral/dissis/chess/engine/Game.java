...
public class Game {
    ...
    public Game move(Position initialPos, Position finalPos, Piece piece, User user) {
        if (finished){
            return this;
        }
       ...
       if(new CheckRule(newBoard).checkMate(newBoard.getColorToPlay())){
        ...
        return new Game(newBoard, new CheckRule(newBoard).checkMate(newBoard.getColorToPlay()), newWinner);
    }
}
